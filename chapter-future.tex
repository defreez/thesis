...
The illustration which concludes the previous chaper, of Alice securely reminding herself to send flowers, may be contrived, but it
...
cone.'' It is unlikely that boundary key mode for eCryptfs, at least in its current form, will ever be widely adopted, but the
following improvements might help.

\paragraph{Configurable Application Security}
...
Each application \texttt{uid} and package name must be hardcoded, which is ridiculous and only meant to demonstrate that the 
boundary keys do function. A user interface needs to be built. This interface could tie to the ``manage applications'' menu and
...
\paragraph{Securely Handle Keys}
...
live memory analysis. This is non-trivial, but certainly possible. The primary hurdle is that very little of the code involved was
...
tool for recovering password keys from memory. Joe Sylve's work could be used \cite{dmd} for validation, but tools have not yet been
released at the time of writing.

\paragraph{Utilize \texttt{ecryptfsd} and encrypt SD card}
...
GID, then it could be cleared. More generally, callling out to \texttt{ecryptfsd} would allow for abitrary logic when
...
As of yet, there has been little exploration the possibility of a memory-only Android device, where no data is ever written. 
...
history, for example, presupposes that a reference tool which recovers browser history is available and can itself succesfully pass
...
is primarily a legal argument for using open-source acquistion and extraction tools, the insights he provides along the way have
...
insufficient or biased testing.  Any reference tool developed purely for the purpose of testing a privacy tool is suspect, as
the tool had not been previously published nor accepted by the forensics community.  This lack of publication and acceptance, in
turn brings into question the validity of the privacy tool, leaving one to wonder whether or not a different forensic tool would be
able to capture the artifacts.  Whenever possible, privacy tools should be validated against forensic techniques developed by
someone other than the author of the privacy tool.  This can be difficult as the field of Android forensics is relatively new, and
there was until very recently little literature on how Android forensics is performed. The field is growing at an astonishing rate,
however, and tools are becoming easier to find.
...
\footnote{http://www.cyanogenmod.com/features/incognito-mode} It does actually do these things, and the browser history removal
has been validated.  Unfortunately, a very important artifact is not being protected against: cache. Failure to remove browser cache
leaves browser history discoverable.

\subsubsection{Browser History Validaton}
...
neither method produces results indicative of a visit to the salt site, nor does the browser using the custom bypass.  
...
Unfortunately the RedPhone service as shutdown after the Twitter acquisition. TextSecure, however, was open-sourced. TextSecure
...
features are included: ``perfect forward secrecy'' and deniability. Perfect forward secrecy\footnote{No, perfect forward secrecy is
not perfect.} uses temporary session keys that are not reproducible if the master key is compromised. So, if Alice sends a message
to Bob, and at a later date the NSA compromises either Alice or Bob's key (or both), the conversation cannot be recreated.
Deniability refers to the ability of the participants in a conversation to deny that the conversation ever happened. In OTR, the
participants are able to authenticate each other during the conversation, but a third-party cannot prove that the conversation
occurred. To be clear, it can be established that \emph{a} conversation occurred between the two participants, but the contents of
that conversation cannot be proven to be authentic \cite{otr}.


\section{Summary}

There is a great deal of work that still needs to be done. The field of mobile forensics is progressing at a rate that far exceeds
the field of mobile privacy. An easy-to-use form of disk encryption that is resistant to live memory analysis is desperately needed.
If securing a device requires intimate knowledge of that device's internals, then in practice it will never be secured. This paper
has taken steps toward building an encryption scheme for Android that may, someday, be resistant to live memory analysis, but it is
certainly not easy to use. Additionally, this encryption project, and all of the other great privacy tools being developed, need
to be rigorously and thoroughly validated. If there are bugs in them, the privacy community needs to find them before an attacker
does.
