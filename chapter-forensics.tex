...
\begin{table}[htb]
\lstset{numbers=none}
\lstinputlisting{tables/adbdevices.txt}
...
More recent Android devices use an ``eMMC'' storage interface. Rather than providing raw access to NAND memory, an eMMC includes a
...
very beginning to run on NAND memory, \texttt{ext4} was optimized for spinning disks. \texttt{YAFFS2} went to great lengths to avoid
overwriting NAND pages. In contrast, \texttt{ext4} overwrites frequently, with no concern for the number of times a physical area of the disk
is written to. Instead, \texttt{ext4} is interested in preventing the head on a hard disk from having to travel too far, and thereby
mitigating the significant latency introduced by mechanical movement. The fact that \texttt{ext4} looks very different From
\texttt{YAFFS2} under the hood matters little to the rest of the operating system, because the Linux kernel provides an
...
of the file system in use. The \texttt{ext4} file system is enormously complex, but the basics are presented here.

The \texttt{ext4} file system is an enhancement to the \texttt{ext3} file system, which in turn was an enhancement to the
\texttt{ext2} file system. The fundamental mechanics of \texttt{ext2}, \texttt{ext3}, and \texttt{ext4} are very similar. Where the
...
The ``smallest addressable storage unit'' of a hard disk is a sector, and for an \texttt{ExtX} file system it is a block
...
an array of 512 byte sectors \cite{linuxdrivers}.  A typical \texttt{ExtX} file system will use 4KiB blocks (eight sectors), but one
of the changes introduced by \texttt{ext4} was support for block sizes of up to 64KiB. Data is read and written to the disk in
multiples of blocks, rather than bytes or bits, which are the units dealt used by applications. \texttt{ExtX} divides a disk into a
series of block groups, the size of which is determined at the time of file system creation.  Each block group contains a copy of
the ``superblock,'' which contains key metadata about the file system.  These copies can be used for recovery should the primary superblock 
become corrupted.  Furthermore, because the metadata associated with a file is almost always held within the same block group as the
actual data for that file, block groups increase the performance of \texttt{ExtX} by minimizing the amount of mechanical
movement required of the hard disk. Additionally, the block group contains a ``group descriptor table,'' which holds metadata about the
layout of the block group itself, and an ``inode table'' with holds information about the files in the block group. In
\texttt{ext4} the block group layout has been modified with the introduction of ``flex block groups,'' which centralize the metadata
associated with a number of block groups, but this scalability improvement does not alter the basic ideas behind block groups \cite{ext4implications}.

Each file has a piece of metadata associated with it called an inode, and the inodes for each block group are stored in the inode table within
the block group. All of the information about a file, excluding the filename and the actual contents of the file, are stored in the
inode. The inode structure includes, for example, the user ID (UID), the permissions mode, and pointers to the actual data \cite[p.
457]{carrier}. Chapter \ref{ch:ecryptfs} uses the fact that each inode has a UID associated with it to derive an encryption key for
the contents of the file. Directories, or folders, in \texttt{ExtX} are simply files of a special type \cite[p. 424]{carrier}. The
contents of a directory file is a list of directory entries, which are data structures that contain the actual names of the files in
the directory. In \texttt{ext4} the directory entries are stored in a hash tree structure by default, instead of a simple list which
was the default for \texttt{ext2} and \texttt{ext3}. A directory entry contains a pointer to the inode for the file, enabling data
retrieval based on the name of a file. 
...
data blocks are zeroed in \texttt{ext3} and \texttt{ext4}, making it necessary to carve\footnote{Carving is the process of searching
...
\cite{ext4implications}. There has been some discussion of introducing secure delete functionality into \texttt{ext4}, but at the
time of writing it is not supported in most implementations \cite{ext4securedelete}.

There is an incredible amount of complexity necessarily being glossed over here, but the basics of \texttt{ExtX} have been covered.
A traditional spinning disk is addressed in sectors, and a series of sectors makes a block. \texttt{ExtX} contains a superblock,
which is just an area holding metadata about the file system. \texttt{ExtX} divides the disk into a series of block groups, and each
...
with that project developed a kernel module, called the Droid Memory Dumper (DMD), capable of capturing memory from a running
Android device. The module is capable of sending the memory capture either to a local file on the device's SD card or to a remote
system over a network connection. While for some time it has been well known that acquiring live memory from an Android device
should be possible, DMD is the first demonstration of how it can actually be accomplished.
...
though progress is being made toward supporting Linux memory dumps, little work had been previously done on Android. The team that
...
Mounting an ext4 image is much easier. The majority of Linux distributions have moved to using ext4 as their default file system,
and thus come with excellent ext4 support out of the box. An ext4 image can be mounted for perusal by passing the loop mount option,
as shown in Table \ref{tab:loopext4}.
...
\subsection{Carving SQLite Records} A large percentage of forensically interesting information on an Android phone is
