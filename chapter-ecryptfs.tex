...
used to verify message integrity by function. Using an HMAC would be an improvement, though typical attacks against straight hashing
would be difficult here, since the extension that could be added to the master key is limited to the size of a UID. Moving to SHA1
for the hash function would probably be more important, and eCryptfs is moving in that direction. 
